...
def ao_conectar(client, userdata, flags, reason_code, properties=None):
    print(f"Conectado ao broker com status: {reason_code}")
...
def ao_receber_mensagem(client, userdata, msg):
    try:
        ...
        ''', (
            ...
            payload.get("ambiente", {}).get("temperatura"),
            payload.get("ambiente", {}).get("umidade"),
            payload.get("ambiente", {}).get("vento_kmh"),
            inc_x,
            inc_y,
            inc_leste,
            inc_oeste,
            inc_norte,
            inc_sul,
            payload.get("alertas", {}).get("status_global")
        ))

        conn.commit()
        conn.close()

        print(f"[{horario_brasilia}] Dados salvos | Status: {payload.get('alertas', {}).get('status_global')}")
